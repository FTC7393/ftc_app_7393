...
public class Particles extends ArrayList<evlib.vision.processors.Particle> {
    /**
     ...
     */
    public void add(evlib.vision.processors.Particle particle, Angle angle) {
        super.add(new evlib.vision.processors.Particle(
                new Vector2D(
...
//        Log.i("Particle", "added particle angle2: " + Angle.add(particle.getFieldPosition().getDirection(), angle).degrees() + " degrees");
     ...
     * @param angle     the angle the robot is turned at
     */
    public void add(List<evlib.vision.processors.Particle> particles, Angle angle) {
        for (evlib.vision.processors.Particle particle : particles) {
            add(particle, angle);
        }
    }
    ...
    public void removeDuplicates() {
        ...
        for (int i = 0; i < size - 1; i++) {
            evlib.vision.processors.Particle particle1 = this.get(i);
            Vector2D pos1 = particle1.getFieldPosition();
            evlib.vision.processors.Particle.Color color1 = particle1.getColor();
            for (int j = i + 1; j < size; j++) {
                if (i != j) {
                    evlib.vision.processors.Particle particle2 = this.get(j);
    ...
    public List<Vector2D> getMovementsToParticle(TeamColor teamColor) {
        return getMovementsToParticle(evlib.vision.processors.Particle.Color.fromTeamColor(teamColor));
    }
     ...
     * @return a list of Vector2D objects that represents the movements in inches
     */
    public List<Vector2D> getMovementsToParticle(evlib.vision.processors.Particle.Color particleColor) {
...
//        2 movements
    ...
    public Particles getUnobstructedParticles(evlib.vision.processors.Particle.Color particleColor) {
     ...
     */
    public Particles getUnobstructedParticlesAt(Vector2D centerPoint, evlib.vision.processors.Particle.Color particleColor) {
        Particles particles = new Particles();
        for (int i = 0; i < this.size(); i++) {
            evlib.vision.processors.Particle particle = this.get(i);
    ...
    private List<Particles> getObstructionsForParticles(evlib.vision.processors.Particle.Color particleColor, boolean getAll) {
     ...
     */
    private List<Particles> getObstructionsForParticlesAt(evlib.vision.processors.Particle.Color particleColor, Vector2D centerPoint, boolean doGetAll) {
    ...
    private Particles getObstructionsForParticleAt(int i, Vector2D centerPoint, boolean doGetAll) {
        ...
        for (int j = 0; j < this.size(); j++) {
            if (i != j) {
                evlib.vision.processors.Particle particle1 = this.get(j);
     ...
     */
    public evlib.vision.processors.Particle getNearestParticle() {
        double minDistance = Double.MAX_VALUE;
        evlib.vision.processors.Particle nearestParticle = null;
        for (evlib.vision.processors.Particle particle : this) {
     ...
     * @return the closest particle
     */
    public evlib.vision.processors.Particle getFarthestParticle() {
     ...
     */
    private Particles getParticlesOfColor(evlib.vision.processors.Particle.Color particleColor) {
        Particles particles = new Particles();
        for (evlib.vision.processors.Particle particle : this) {
            if (particle.getColor() == particleColor) {
                particles.add(particle);
