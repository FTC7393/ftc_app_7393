 ...
 */
public class HueBeaconProcessor implements evlib.vision.processors.ImageProcessor<evlib.vision.processors.BeaconResult> {
...
//  private static final Scalar HSV_MAX = new Scalar(179, 255, 255);
     ...
     */
    @Override
    public ImageProcessorResult<evlib.vision.processors.BeaconResult> process(long startTime, Mat rgbaFrame, boolean saveImages) {
        ...
        evlib.vision.processors.BeaconFinder beaconFinder = new BeaconFinder();
        ...
        if (positionResult == null) {
            ...
            return new ImageProcessorResult<>(startTime, rgbaFrame, new evlib.vision.processors.BeaconResult(new evlib.vision.processors.BeaconColorResult(), null));
        }
        ...
        Mat subHSV;
        Scalar averageRGB;
        evlib.vision.processors.BeaconColorResult.BeaconColor[] beaconColors = new evlib.vision.processors.BeaconColorResult.BeaconColor[2];
...
//            Log.i(TAG, "HSV " + i + "," + j + ": " + data[j]);
            ...
            if (area == 0 || hsvAverage[1] < S_MIN || hsvAverage[2] < V_MIN) {
                beaconColors[i] = evlib.vision.processors.BeaconColorResult.BeaconColor.UNKNOWN;
                averageRGB = ImageUtil.BLACK;
            } else {
                int hue = (int) ((2 * hsvAverage[0]) % 360);
                if (hue < 0) hue += 360;
                if (hue >= 300 + GAP || hue <= 60 - GAP) {
                    beaconColors[i] = evlib.vision.processors.BeaconColorResult.BeaconColor.RED;
                } else if (hue >= 60 + GAP && hue <= 180 - GAP) {
                    beaconColors[i] = evlib.vision.processors.BeaconColorResult.BeaconColor.GREEN;
                } else if (hue >= 180 + GAP && hue <= 300 - GAP) {
                    beaconColors[i] = evlib.vision.processors.BeaconColorResult.BeaconColor.BLUE;
                } else {
                    beaconColors[i] = evlib.vision.processors.BeaconColorResult.BeaconColor.UNKNOWN;
                }
